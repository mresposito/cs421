...
let translate (pic:picture) (offset:point) : picture
    = fun phi -> pic ( fun pt -> phi (offset ++ pt))
...
let shorten (lf:linefun) : linefun
  = fun (p1:point) -> fun (p2:point) -> (
    let delta = ( p2 -- p1 ) *** (0.25, 0.25)
    in lf ( p1 ++ delta ) ( p2 -- delta )
  )
...
let nodelist (pic:picture) (w0:float) (h0:float)
 ...
 = let pics tlis    = List.map (fun (p,_,_) -> p) tlis
   and widths tlis  = List.map (fun (_,w,_) -> w) tlis
   and heights tlis = List.map (fun (_,_,h) -> h) tlis
   and sum lis = List.fold_left (fun a b -> a +. b) 0.0 lis
   and maxelt lis =  List.fold_left (fun a b -> max a b) 0.0 lis 
   and mkpairs lis1 lis2 = List.combine lis1 lis2
   and draw_pics pics locs = 
     List.map2 ( fun pic loc -> translate pic loc ) pics locs
   and draw_lines p pts = List.map (fun pt -> line p pt) pts
   in let vsep = h0 /. 4.0
